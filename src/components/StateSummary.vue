...
export default {
    ...
    computed: {
        winnersByState () {      
            const data = this.winnersData
            const filtered = []
            for (const key in data) {
                if (data[key].statename === this.storeSelectedState) { filtered.push(data[key]) }
            }
            return filtered
        },
        groupByPartySum () {
            const groupByPartySum = this.winnersByState.reduce((acc, it) => {
                const win = +it.win
